...
### myNetflix 앱</br>
   - AVFoundation을 이용하여 구현(동영상 재생과 관련됐기 때문)
   - Firebase를 연동하여 Searchbar에 검색 기록을 저장해준다.
   - SearchAPI를 구현하여 실제로 서버에 있는 동영상을 불러와 재생시킴.
   - container view에 네스티드 scroll view를  구현해 각각에 들어갈 영화 포스터를 나눠주었다.
